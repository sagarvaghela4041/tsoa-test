...
var SpecGenerator3 = /** @class */ (function (_super) {
    ...
    SpecGenerator3.prototype.buildMethod = function (controllerName, method, pathObject) {
        ...
        var formParams = method.parameters.filter(function (p) { return p.in === 'formData'; });
    ...
    SpecGenerator3.prototype.removeDuplicateSwaggerTypes = function (types) {
        ...
        if (types.length === 1) {
            ...
            return Array.from(typesSet).map(function (typeString) { return JSON.parse(typeString); });
        }
    };
    SpecGenerator3.prototype.getSwaggerTypeForUnionType = function (type) {
        var _this = this;
        var notNullSwaggerTypes = this.removeDuplicateSwaggerTypes(this.groupEnums(type.types.filter(function (x) { return !_this.isNull(x); }).map(function (x) { return _this.getSwaggerType(x); })));
        var nullable = type.types.some(function (x) { return _this.isNull(x); });
        if (nullable && notNullSwaggerTypes.length === 1) {
            var _a = __read(notNullSwaggerTypes, 1), swaggerType = _a[0];
        ...
        if (nullable) {
            if (notNullSwaggerTypes.length === 1) {
                var _b = __read(notNullSwaggerTypes, 1), swaggerType = _b[0];
